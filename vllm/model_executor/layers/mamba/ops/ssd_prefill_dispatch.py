...
def _load_flashinfer_ssd_class() -> type:
    ...
    if not getattr(SSDCombined, "supports_strided_intermediate_states", False):
        raise RuntimeError(
            "FlashInfer SSDCombined API v2 must support strided intermediate states"
        )

    required_init = {"has_intermediate_states", "output_token_major"}
    required_run = {
        "num_seqs",
        "valid_seqlen",
...
def run_flashinfer_mamba2_prefill(
    ...
    if (
        ...
        or request.valid_seqlen != request.x.shape[0]
        ...
        or request.intermediate_state_indices.shape != request.chunk_indices.shape
    ...
    if request.padded_seqlen == request.valid_seqlen:
        ...
        dt = request.dt.new_zeros(1, request.padded_seqlen, request.dt.shape[1])
        ...
        x[:, : request.valid_seqlen].copy_(request.x)
        dt[:, : request.valid_seqlen].copy_(request.dt)
        B[:, : request.valid_seqlen].copy_(request.B)
        C[:, : request.valid_seqlen].copy_(request.C)
    ...
    if not request.is_warmup:
        ...
        _STATS.flashinfer_layer_tokens += request.valid_seqlen
        logger.info_once(
            "FlashInfer Mamba2 SSD prefill engaged: T=%d, padded_T=%d, "
            "num_seqs=%d, logical_segments=%d.",
            request.valid_seqlen,
